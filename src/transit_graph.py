...
def best_route_shortest_walk_distance(dict_trip, key_name):
    shortest_distance = maxint
    nearest_key = -1
...
def station_location_shortest_walk_distance(g_transit, origin_station, destination_location):
    ...
    dict_last_station = best_route_shortest_walk_distance(dict_line_stations_near_destination, 'line')

    path_stations = nx.shortest_path(g_transit, origin_station, dict_last_station['station'],\
     weight='distance')
    path_length = nx.shortest_path_length(g_transit, origin_station, dict_last_station['station'],\
     weight='distance')
...
def travel_distance_shortest_walk_distance(first_subway_boarding, ct_origin,\
    ...
    initial_walking_distance = vincenty((origin_centroid.iloc[0].x, origin_centroid.iloc[0].y),\
     g_transit.node[first_subway_boarding]['posxy']).meters
    #print 'origin_boarding_distance', initial_walking_distance

    travel = station_location_shortest_walk_distance(g_transit, first_subway_boarding, destination_centroid)
